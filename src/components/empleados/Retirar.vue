  ...
  <div class="container">
    <div class="container2">
      ...
      <label for="numeroReserva"
        >Ingrese el número para retirar la reserva del vehiculo:</label
      >
      <tr />
      <input type="text" id="numeroReserva" v-model="reserva" />
      <button @click="retirarVehiculo()">Retirar</button>
...
<script>
import { buscarVehiculoReservadoFachada } from "@/helpers/clienteReserva";
...
export default {
  ...
  methods: {
    async retirarVehiculo() {
      const reserv = this.reserva;
      try {
        this.Reserva = await buscarVehiculoReservadoFachada(reserv);
